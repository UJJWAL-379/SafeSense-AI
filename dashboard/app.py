...
def analyze_reports(reports):
    r = requests.post(f"{API_URL}/cluster", json={"reports": reports}, timeout=120); r.raise_for_status(); return r.json()
...
def submit_feedback(cluster_id, decision, comment):
    r = requests.post(f"{API_URL}/feedback", json={"cluster_id": str(cluster_id), "decision": decision, "comment": comment}, timeout=20); r.raise_for_status(); return r.json()

reports = load_demo()
with st.sidebar:
    st.header("🎛️ Demo Controls")
    mode = st.radio("Demo mode", ["Live report", "Precursor cluster demo"], index=1)
    st.divider(); st.caption("Judge flow: ingest → understand → semantically connect → alert → human verification.")
    st.caption("🇮🇳 Indian context: configurable for applicable Factories Act, DGMS and BIS requirements.")
...
if mode == "Live report":
    ...
    uploaded = st.file_uploader("Or upload TXT / PDF / Excel / image", type=["txt", "pdf", "xlsx", "xls", "png", "jpg", "jpeg", "webp"])
    if st.button("🔎 Analyze Report", type="primary", use_container_width=True):
        try:
            ...
            st.success("Report analyzed and persisted to SQLite")
            st.json(signal)
        except Exception as exc: st.error(f"Analysis failed: {exc}")
else:
    st.markdown('<div class="demo-box"><b>Judge demo:</b> These reports come from different channels and look minor individually. SafeSense connects them using semantic similarity, so different wording can still reveal the same emerging risk.</div>', unsafe_allow_html=True)
    ...
    if st.button("🚨 Detect Emerging Safety Risks", type="primary", use_container_width=True):
        try:
            with st.spinner("Running NLP extraction and semantic precursor detection..."): st.session_state["result"] = analyze_reports(chosen)
    ...
    if result:
        ...
        if alert.get("sent"):
            st.success("🔔 HIGH/CRITICAL alert sent to the configured webhook.")
        elif alert.get("would_alert"):
            st.info("🔔 Alert condition reached. Configure ALERT_WEBHOOK_URL to send it to Slack/Teams/email gateway.")
        ...
        if clusters:
            for cluster in clusters:
                ...
                with st.expander(f"{icon} {level} — {cluster['label']} — {cluster['count']} linked reports", expanded=(level == "CRITICAL")):
                    ...
                    st.write(f"**Similarity:** {cluster.get('similarity_method','semantic matching')}")
                    st.write(f"**Action:** {cluster.get('recommendation','—')}")
                    st.write(f"**Reports:** {', '.join(cluster['report_ids'])}")
                    st.markdown("**Safety officer verification**")
                    comment = st.text_input("Optional comment", key=f"comment-{cluster['cluster_id']}")
                    a,b = st.columns(2)
                    if a.button("✅ Confirm risk", key=f"confirm-{cluster['cluster_id']}"):
                        try: submit_feedback(cluster["cluster_id"], "confirmed", comment); st.success("Feedback saved — risk confirmed.")
                        except Exception as exc: st.error(f"Feedback failed: {exc}")
                    if b.button("❌ Reject risk", key=f"reject-{cluster['cluster_id']}"):
                        try: submit_feedback(cluster["cluster_id"], "rejected", comment); st.success("Feedback saved — cluster rejected.")
                        except Exception as exc: st.error(f"Feedback failed: {exc}")
        else: st.info("No repeated precursor cluster detected.")
        st.subheader("🎯 The SIH 'aha' moment")
        st.write("A single report can look like a routine near-miss. SafeSense uses semantic similarity to connect differently worded reports and asks: **Is the same hazard recurring around the same place or asset?** When the pattern crosses a threshold, it becomes an early-warning signal for the safety team.")
