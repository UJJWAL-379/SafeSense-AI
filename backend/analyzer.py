...
HAZARDS={"fall":["fall","scaffold","scaffolding","ladder","guard rail","guardrail","height","wobbl","bolt"],"electrical":["electrical","electric","electrocution","shock","wire","cable","panel","spark","live wire"],"chemical":["chemical","solvent","gas","hcl","toxic","spill","leak","acid"],"fire":["fire","smoke","flame","ignition","spark"],"equipment_failure":["machine","equipment","pump","valve","brake","failure","malfunction","damaged"],"lifting":["crane","lifting","load","rigging","hoist"],"housekeeping":["oil spill","slip","trip","obstruction","housekeeping"]}
SEVERITY_WEIGHT={"low":10,"medium":30,"high":60,"critical":85,"near-miss":25}

def _contains(text:str,words:list[str])->list[str]: return [w for w in words if w.lower() in text.lower()]
...
def _clean(text:str)->str:
    ...
    out=text
    for src,dst in replacements.items(): out=re.sub(rf"\b{re.escape(src)}\b",dst,out,flags=re.I)
    return re.sub(r"\s+"," ",out).strip()

def _extract_location(text:str)->str:
    zone=re.search(r"\b(zone\s+[A-Za-z0-9]+)",text,flags=re.I); tower=re.search(r"\b(tower\s+[A-Za-z0-9]+)",text,flags=re.I)
    if zone and tower:return f"{zone.group(1).title()} · {tower.group(1).title()}"
    if zone:return zone.group(1).title()
    return "unknown"
...
def fallback_extract(text:str,report_id:str|None=None)->SafetySignal:
    original=text or ""; t=_clean(original).lower()
    hazard_scores={h:len(_contains(t,w)) for h,w in HAZARDS.items()}; hazard=max(hazard_scores,key=hazard_scores.get) if max(hazard_scores.values()) else "unknown"
    ...
    root_causes=[]
    cause_map={"missing guard rail":["missing guard rail","guard rail missing"],"structural failure":["loose scaffolding","scaffolding loose","wobbling","bolt missing"],"bolt missing":["bolt missing","missing bolt"],"worker fatigue":["fatigue","12hr shift","12 hr shift"],"LOTO violation":["loto not followed","lock out tag out not followed","lock out tag out"],"PPE non-compliance":["no ppe","without ppe","ppe kit unavailable","personal protective equipment"],"damaged cable":["damaged cable","insulation damaged","exposed wire"],"poor housekeeping":["oil spill","oil rags","slipped near storage"],"expired equipment":["expired","checked date 2023"]}
    for cause,phrases in cause_map.items():
        if any(p in t for p in phrases):root_causes.append(cause)
    base=SEVERITY_WEIGHT[severity]; repetition_bonus=15 if any(x in t for x in ["2nd time","3rd complaint","again","repeat"]) else 0; exposure_bonus=10 if any(x in t for x in ["exposed","live wire","may collapse","sparks"]) else 0
    score=min(100,base+repetition_bonus+exposure_bonus+min(10,len(found)*2)); urgency="immediate" if score>=80 else "urgent" if score>=55 else "monitor"
...
def llm_extract(text:str,report_id:str|None=None)->SafetySignal:
    ...
    try:
        ...
        data:dict[str,Any]=json.loads(response.choices[0].message.content); data["report_id"]=report_id; return SafetySignal.model_validate(data)
    except Exception:return fallback_extract(text,report_id)

def cluster_signals(signals:list[SafetySignal])->list[dict[str,Any]]:
    groups:dict[str,list[SafetySignal]]={}
    for s in signals:
        key=f"{s.hazard_type}|{(s.location or 'unknown').strip().lower()}"; groups.setdefault(key,[]).append(s)
    results=[]
    for idx,(key,items) in enumerate(groups.items(),start=1):
        if len(items)<2:continue
        causes=Counter(c for s in items for c in s.root_causes+s.precursor_terms); avg_score=round(sum(s.risk_score for s in items)/len(items)); repetition=sum(1 for s in items if any(x in s.precursor_terms for x in ["again","repeat","complaint"]))
